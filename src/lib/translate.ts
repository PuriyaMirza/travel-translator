...
// claude-opus-5. SPEC.md §3 says to check the current model string rather
// than assume one — this is it as of the current API.
const MODEL = "claude-opus-5";

// Sampling parameters (temperature/top_p/top_k) are removed on claude-opus-5
// and return a 400 — SPEC.md §8's "temperature around 0.3" doesn't apply to
// the current API. Consistency instead comes from a tight prompt plus
// structured outputs, with a low effort level: this is a short,
// well-specified transformation, not a task that benefits from deep
// reasoning.
const EFFORT = "low";

// Thinking tokens count toward max_tokens, and thinking is on by default on
// claude-opus-5. SPEC.md §8's "~1024" is sized for the JSON response alone
// and would risk truncation; 2048 leaves headroom without disabling thinking
// (which has its own failure modes on this model).
